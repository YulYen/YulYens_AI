...
- **LoRA-Finetuning (PoC):** Erste Experimente zur Modellverfeinerung sind bereits enthalten. Im Verzeichnis `tranining/` befindet sich ein kleines **LoRA-Finetuning**-Beispiel, das zeigt, wie man mittels [PEFT/QLoRA](https://github.com/huggingface/peft) einen kompakten Adapter trainieren kann. In diesem Proof-of-Concept wurde die Persona Doris mit ca. 200 Frage-Antwort-Paaren feingetunt. Ein passendes Test-Skript überprüft anschließend die Wirksamkeit des trainierten Adapters. Diese Funktion ist experimentell und nicht in den Hauptbetrieb integriert, zeigt aber die Ausbaufähigkeit des Systems.  
