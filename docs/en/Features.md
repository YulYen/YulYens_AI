...
- **Answer feedback (👍/👎):** Every answer can be rated in the web UI. Each click appends a line to `data/feedback_votes.jsonl` — **next to the store, not in `logs/`**: anything in `logs/` may be cleared out at any time, whereas collected ratings are human judgement and cannot be reproduced. The location follows the directory of `storage.path`; a file left at the old location in `logs/` is adopted automatically on first use. Each line carries a timestamp, persona, model, question, answer, the vote and a reference to the stored conversation. The file is append-only (changing your mind adds a line rather than replacing the old one), so the history stays auditable. Intended as a data basis for quality comparisons and later finetuning.
