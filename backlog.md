...
| 9 | Ask-all (broadcast): WebUI Wiki + Streaming | TerminalUI-Flow läuft. **Live-Streaming im WebUI DONE (2026-07-04): `iter_broadcast_events` in `orchestrator.py` streamt tokenweise, Antworten erscheinen live in Markdown-Sektionen.** Offen bleibt: Wiki-Kontext im Broadcast (der Orchestrator kennt kein Wiki). | S | M | Orchestration |
