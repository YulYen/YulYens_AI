...
def _build_user_visible_link(host_header: str, term: str, online: bool) -> str:
    """
...
# ---------- Anfrage-Auswertung (ohne Socket, damit testbar) ---------------------
@dataclass(frozen=True)
class ProxyResponse:
    """Was der Proxy antworten würde — als Wert statt direkt auf den Socket.

    Vorher steckte die ganze Auswertung in ``do_GET``; getestet war davon
    nichts (20 % Deckung), weil man dafür einen echten Handler samt Socket
    gebraucht hätte.
    """
...
def handle_lookup(path: str, host_header: str = "localhost") -> ProxyResponse:
    """Wertet eine Proxy-Anfrage aus: Pfad + Query rein, Antwort raus."""
    ...
    logger.info(f"[Request] term='{search_term}' path='{path}' online={online}")

    if not search_term:
        return ProxyResponse(400, text="Search term missing. Example: /Albert_Einstein")
    ...
    if status != 200:
        if status == 404:
            ...
            return ProxyResponse(404, text="Article not found.")
        logger.error(f"[Error] HTTP status {status} for '{search_term}'")
        return ProxyResponse(500, text=f"Unexpected error – HTTP status: {status}")
    ...
    # Target link & UI hint
    link = _build_user_visible_link(host_header, search_term, online)
...
class WikiRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            result = handle_lookup(self.path, self.headers.get("Host", "localhost"))
            if result.payload is not None:
                _send_json(self, result.status, result.payload)
            else:
                _send_text(self, result.status, result.text)
        finally:
            # Always log the total duration. Bewusst self.path statt der Query:
            # die stand hier vorher und war bei einem frühen Fehler ungebunden.
            duration_total = (time.perf_counter() - start_total) * 1000
            logger.info(
                f'[WikiProxy] Request "{self.path}" answered in {duration_total:.1f} ms'
            )
